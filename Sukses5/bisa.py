...
EXCEL_OUTPUT_FILENAME = "tetsing.xlsx" 
...
if __name__ == "__main__":
    ...
    chroma_embedding_function = embedding_functions.SentenceTransformerEmbeddingFunction(
        ...
        device=MODEL_KWARGS.get('device', 'cpu') # Ambil device dari konfigurasi Anda
    )
    ...
    for pdf_name in tqdm(pdf_files, desc="Memproses semua PDF", unit="file"):
        ...
        if final_chunks:
            ...
            try:
                collection.add(
                    documents=documents, 
                    embeddings=embeddings_for_db, # <-- INI PERUBAHAN UTAMANYA
                    metadatas=metadatas, 
                    ids=ids
                )
