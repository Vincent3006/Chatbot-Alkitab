...
class CustomRAGPipeline:
    ...
    def retrieve_documents(self, question: str, k: int = 4):
        """
        Melakukan retrieval langsung ke ChromaDB untuk mendapatkan skor Cosine Similarity murni.
        (Nilai tinggi = semakin mirip).
        """
        print(f"   > Melakukan retrieval MURNI untuk pertanyaan: '{question}'")

        # 1. Buat embedding untuk pertanyaan
        question_embedding = self.embedding_model.embed_query(question)
        
        # 2. Query langsung ke koleksi ChromaDB, melewati wrapper LangChain
        query_results = self.vectorstore._collection.query(
            query_embeddings=[question_embedding],
            n_results=k
        )
        
        # 3. Olah hasil mentah dari ChromaDB
        results_list = []
        
        ids = query_results.get('ids', [[]])[0]
        documents = query_results.get('documents', [[]])[0]
        metadatas = query_results.get('metadatas', [[]])[0]
        distances = query_results.get('distances', [[]])[0] # Ini adalah Cosine Distance

        for i in range(len(ids)):
            # Konversi Jarak Cosine (0-2) ke Kemiripan Cosine (1 s/d -1)
            # Rumus: Similarity = 1 - Distance
            similarity_score = 1 - distances[i]

            results_list.append({
                "content": documents[i],
                "metadata": metadatas[i],
                "score": similarity_score  # <-- Sekarang ini adalah skor similarity murni
            })
