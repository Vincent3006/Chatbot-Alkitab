        ...
        if os.path.exists(config.DB_NAME):
            ...
            for pdf_name in tqdm(pdf_files, desc="Memproses PDF untuk database"):
                ...
                final_chunks = semantic_chunking(
                    extracted_data, 
                    self.embedding_model, # self.embedding_model sekarang adalah instance QwenEmbedder
                    config.SIMILARITY_THRESHOLD,
                    config.MAX_CHARACTERS_PER_CHUNK,
                    config.BATCH_SIZE_EMBEDDING # Argumen ini tidak lagi digunakan oleh QwenEmbedder, tapi tidak apa-apa
                )
                if final_chunks:
