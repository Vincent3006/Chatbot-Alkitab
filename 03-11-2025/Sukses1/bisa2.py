...
EXCEL_OUTPUT_FILENAME = "tetsing2.xlsx" 
...
def process_data_normal_semantic_chunking(data, hf_embeddings, similarity_threshold=0.4, max_characters=1000):
    if not data:
        return []
    ...
    # Loop batching dan tqdm dihilangkan.
    # Sekarang kita membuat embedding untuk SEMUA teks sekaligus dalam satu panggilan.
    # HATI-HATI: Ini bisa menyebabkan error kehabisan memori (MemoryError) jika file PDF besar.
    print(f"  Membuat embedding untuk semua {len(texts)} kalimat sekaligus (tanpa batching)...")
    try:
        all_embeddings = hf_embeddings.embed_documents(texts)
    except Exception as e:
        print(f"  [ERROR] Gagal membuat embedding. Kemungkinan karena kehabisan memori: {e}")
        return []
    # --- AKHIR PERUBAHAN ---
...
if __name__ == "__main__":
    ...
    for pdf_name in tqdm(pdf_files, desc="Memproses semua PDF", unit="file"):
        ...
        print("\n\n--- MEMULAI INSPEKSI FINAL SEBELUM MASUK DATABASE ---")
        if final_chunks:
            ...
            try:
                ...
                print(f"  Berhasil! {len(final_chunks)} potongan (chunks) dari {pdf_name} ditambahkan dengan embedding yang benar.")
            except Exception as e:
