...
import { FaPaperPlane, FaTimes } from 'react-icons/fa'; // Import FaTimes untuk ikon close
...
const ChatInterface = () => {
  ...
  // Menyimpan sumber yang akan ditampilkan di modal. null = modal tertutup.
  const [modalSources, setModalSources] = useState(null);
  // Menyimpan indeks sumber yang sedang diperluas di dalam modal
  ...
  const handleSend = async () => {
    ...
    // Pastikan modal tertutup saat mengirim pesan baru
  ...
  const closeModal = () => {
    ...
    setExpandedSourceIndex(null); // Reset saat modal ditutup
  };
  ...
  return (
    <div className={styles.chatContainer}>
      {/* --- MODAL DIRENDER DI SINI (DI LUAR AREA CHAT) --- */}
      {modalSources && (
      ...
      <div className={styles.chatMessages} ref={chatMessagesRef}>
        {messages.map((msg) => (
          <div key={msg.id} className={`${styles.messageBubble} ${styles[msg.sender]}`}>
            ...
            {msg.sender === 'ai' && msg.sources && msg.sources.length > 0 && (
              <>
                {/* Tampilan sumber awal (hanya beberapa) */}
        ...
        {isLoading && (
          <div className={`${styles.messageBubble} ${styles.ai} ${styles.loading}`}>
             {/* Tampilan loading dengan efek titik-titik */}
            <p>
              AI sedang mengetik
