...
from utils import extract_text_with_metadata,fixed_size_chunking
...
class CustomRAGPipeline:
    ...
    def _export_database_to_excel(self, vectorstore):
        """
        Mengekspor seluruh isi database ke file Excel dengan kolom yang disesuaikan
        untuk metode fixed-size chunking.
        """
        print("\n" + "="*50)
        print(f"MEMULAI EKSPOR DATABASE (FIXED CHUNKING) KE: {config.EXCEL_OUTPUT_FILENAME}")
        try:
            collection = vectorstore._collection
            total_docs = collection.count()
            if total_docs == 0:
            ...
            print(f"Mengambil {total_docs} dokumen dari database...")
            # Mengambil semua data dari koleksi ChromaDB
            ...
            data_for_excel = []
            # Loop melalui setiap dokumen yang diambil
            for i in range(len(all_data['ids'])):
                metadata = all_data['metadatas'][i]
                
                # Membuat baris data untuk Excel dengan kolom yang relevan
                data_for_excel.append({
                    "Nomor": i + 1,
                    "source_file": metadata.get('source_pdf', 'N/A'),
                    "source_range": metadata.get('source_range', 'N/A'),
                    "isi_ayat": all_data['documents'][i],
                    "jumlah_karakter_chunk": metadata.get('final_char_count', 0),
                    # Dua baris di bawah ini dihapus karena tidak ada lagi di metadata
                    # "skor_kemiripan_internal": metadata.get('internal_similarity_scores', ''), 
                    # "skor_kemiripan_pemisah": metadata.get('break_similarity_score', -1.0)
                })
            ...
            # Menghapus file Excel lama jika ada untuk memastikan file baru yang dibuat
            if os.path.exists(config.EXCEL_OUTPUT_FILENAME):
                os.remove(config.EXCEL_OUTPUT_FILENAME)
                print(f"File '{config.EXCEL_OUTPUT_FILENAME}' yang sudah ada telah dihapus.")
    ...
    def _load_or_create_vectorstore(self):
        ...
        if os.path.exists(config.DB_NAME):
            ...
            for pdf_name in tqdm(pdf_files, desc="Memproses PDF untuk database"):
                ...
                # )
                final_chunks = fixed_size_chunking(
                    data=extracted_data, 
